...
geometry_msgs::msg::TransformStamped tf2tfStamped_msg(const tf2::Transform& transform, const std::string& parent_frame_id, const std::string& child_frame_id)
{
    geometry_msgs::msg::Transform transform_msg = tf2::toMsg(transform);
    ...
    : kinova_robotType_(kinova_robotType)
{
    ...
    {
        RCLCPP_WARN(rclcpp::get_logger("kinova_arm_kinematics"), "Invalid kinova_robotType error! Obtained: %s.", kinova_robotType_.c_str());
        return;
    }
...
//    tf_prefix_ = kinova_robotType_ + "_" + boost::lexical_cast<string>(same_type_index); // in case of multiple same_type robots
    ...
    if (kinova_robotType_.substr(0,4) == "j2n4" || kinova_robotType_.substr(0,4) == "m1n4")
    {
        ...
        D1_ = 0.2755;
        if(!node_handle->has_parameter("D1")){
            node_handle->declare_parameter("D1", D1_);
        }
        node_handle->get_parameter("D1", D1_);
        if(robot_category_ == 'j') {
            D2_ = 0.41;
            D3_ = 0.2073;
            e2_ = 0.0098;
            if(!node_handle->has_parameter("D2")){
                node_handle->declare_parameter("D2", D2_);
            }
            if(!node_handle->has_parameter("D3")){
                node_handle->declare_parameter("D3", D3_);
            }
            if(!node_handle->has_parameter("e2")){
                node_handle->declare_parameter("e2", e2_);
            }
            node_handle->get_parameter("D2", D2_);
            node_handle->get_parameter("D3", D3_);
            node_handle->get_parameter("e2", e2_);
        }
        else
        {
            D2_ = 0.29;
        ...
        D4_ = 0.160;
        if(!node_handle->has_parameter("D4")){
            node_handle->declare_parameter("D4", D1_);
        }
        node_handle->get_parameter("D4", D4_);
        wrist_deg_ = 60.0;
        if(!node_handle->has_parameter("wrist_deg")){
            node_handle->declare_parameter("wrist_deg", wrist_deg_);
        }
        node_handle->get_parameter("wrist_deg", wrist_deg_);
    ...
    else if (kinova_robotType_.substr(0,4) == "j2n6" || kinova_robotType_.substr(0,4) == "m1n6" )
    {
        ...
        D6_ = 0.160;
        if(!node_handle->has_parameter("D4")){
            node_handle->declare_parameter("D4", D2_);
        }
        if(!node_handle->has_parameter("D5")){
            node_handle->declare_parameter("D5", D2_);
        }
        if(!node_handle->has_parameter("D6")){
            node_handle->declare_parameter("D6", D2_);
        }
        node_handle->get_parameter("D4", D4_);
        node_handle->get_parameter("D5", D5_);
        node_handle->get_parameter("D6", D6_);
        wrist_deg_ = 60.0;
        if(!node_handle->has_parameter("wrist_deg")){
            node_handle->declare_parameter("wrist_deg", wrist_deg_);
        }
    ...
    else if (kinova_robotType_.substr(0,4) == "j2s6")
    {
        ...
        if(!node_handle->has_parameter("D4")){
            node_handle->declare_parameter("D4", D4_);
        }
        if(!node_handle->has_parameter("D5")){
            node_handle->declare_parameter("D5", D5_);
        }
        if(!node_handle->has_parameter("D6")){
            node_handle->declare_parameter("D6", D6_);
        }
        if(!node_handle->has_parameter("e2")){
            node_handle->declare_parameter("e2", e2_);
        }
    ...
    else if (kinova_robotType_.substr(0,4) == "j2s7")
    {
        ...
        if(!node_handle->has_parameter("D7")){
            node_handle->declare_parameter("D7", D7_);
        }
        if(!node_handle->has_parameter("e2")){
            node_handle->declare_parameter("e2", e2_);
        }
        ...
        node_handle->get_parameter("D5", D5_);
        node_handle->get_parameter("D6", D6_);
        node_handle->get_parameter("D7", D7_);
        node_handle->get_parameter("e2", e2_);
    ...
    // iniial the tf broadcaster
    broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*node_handle);
}
...
void KinovaKinematics::updateForward(float* Q)
{
    ...
    broadcaster_->sendTransform(tf2tfStamped_msg(transform, baseFrame, concatTfName(tf_prefix_, "link_base")));
    ...
    for (int i = 0; i<arm_joint_number_; i++)
    {
        ...
        if(i==0)
        {
            broadcaster_->sendTransform(tf2tfStamped_msg(transform, concatTfName(tf_prefix_, "link_base"), concatTfName(tf_prefix_, "link_1")));
        }
        else
        {
            broadcaster_->sendTransform(tf2tfStamped_msg(transform, concatTfName(tf_prefix_, "link_", i), concatTfName(tf_prefix_, "link_", i+1)));
        }
    }
